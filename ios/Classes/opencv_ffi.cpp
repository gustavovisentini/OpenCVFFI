...
extern "C"
{
    ...
    __attribute__((visibility("default"))) __attribute__((used)) const char *process_image(char *inputImagePath, char *outputImagePath)
    {
        ...
        try
        {
            ...
            imagem_original = imread(inputImagePath);
            ...
            // cv::cvtColor(img, img_hsv, COLOR_GRAY2BGR);
            // cv::cvtColor(img, img_hsv, COLOR_RGB2HSV);
            ...
            cv::circle(img, center, radius, cv::Scalar(0, 0, 0), cv::FILLED);
            ...
            std::string result = to_string(total_areaIAF) + "|" + to_string(qtdeFolhas) + "|" + to_string(menorFolha) + "|" + to_string(maiorFolha);

            return result.c_str();
            // return strcat('total_areaIAF', strcat(strcat(strcat(total_areaIAF, qtdeFolhas), menorFolha), maiorFolha));
            // return "oi";
        }
        catch (...)
        {
            // platform_log(std::string("Error: ") + str::string(e.what()));
            // platform_log(strcat("Error: ", "..."));
            return "Erro ao coisar a imagem";
        }
    }
}
